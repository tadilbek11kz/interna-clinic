...
from app.models.models import Patient, PatientTest
from app.schemas.schemas import PatientRead, PatientCreateRawPassword, PatientCreateHashedPassword, \
    PatientUpdateRawPassword, PatientUpdateHashedPassword, PatientTestRead, PatientTestCreate, PatientTableProperties
...
class PatientService:
    ...
    async def download_patients(self, token: str, patient_data_properties: PatientTableProperties) -> dict[str, Any]:
        """
        ...
        return await self.patient_repository.download_patients(patient_data_properties)
    ...
    async def create_patient_test(self, test_data: PatientTest, patient_id: int, token: str) -> dict[str, Any]:
        """
        This method is used to create a test for the patient with the given ID.

        Returns:
            created test (PatientTest)
        """
        try:
            verify_token(token)
        except JWTError:
        ...
        patient_test = await self.patient_repository.create_patient_test(test_data, patient_id)
        if not patient_test:
            raise HTTPException(status_code=404, detail=f"Test with this data for pattient cannot be created does not exist.")

        return patient_test

    async def get_patient_tests(self, patient_id: int, token: str) -> tuple[Any | None, Sequence[PatientTest]]:
        """
        This method is used to retrieve all tests for the patient with the given ID.

        Returns:
            total (int)
            tests (Sequence[PatientTest])
        """
        try:
            verify_token(token)
        except JWTError:
        ...
        total, tests = await self.patient_repository.get_patient_tests(patient_id)
        return total, tests

    async def get_patient_test_by_name(self, patient_id: int, test_name: str, token: str) -> Sequence[PatientTest] | None:
        """
        This method is used to retrieve a certain test for the patient with the given ID by its 'name' field.

        Returns:
            test (Sequence[PatientTest] | None)
        """
        ...
        tests = await self.patient_repository.get_patient_test_by_name(patient_id, test_name)
        if not tests:
            raise HTTPException(status_code=404, detail=f"Test with name {test_name} does not exist.")

        return tests

    async def get_patient_test_names(self, patient_id: int, token: str) -> Sequence[str]:
        """
        This method is used to retrieve all test names for the patient with the given ID.

        Returns:
            test names (Sequence[str])
        """
        try:
            verify_token(token)
        except JWTError:
        ...
        test_names = await self.patient_repository.get_patient_test_names(patient_id)
        return test_names
